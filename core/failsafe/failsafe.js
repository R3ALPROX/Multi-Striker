...
const { getBotName } = require("../../config/identity");
...
function recordFailure(guildId, source, error) {
    ...
    if (recent.length >= 8) enterSafeMode(guildId, "Repeated internal failures detected");
}
...
function enterLockdown(guildId, reason) { return setState(guildId, "LOCKDOWN", reason); }
function emergencyShutdown(guildId, reason) { return setState(guildId, "EMERGENCY_SHUTDOWN", reason); }
function recover(guildId) { return setState(guildId, "NORMAL", "Owner-authorized recovery"); }

async function notifyOwner(guild, state) {
    try {
        const owner = await guild.fetchOwner();
        await owner.send(
            `🚨 **${getBotName()} Fail-Safe Activated**\n` +
            `Server: **${guild.name}**\n` +
            `Mode: **${state.mode}**\n` +
...
module.exports = { recordFailure, enterSafeMode, enterLockdown, emergencyShutdown, recover, trigger, getState };
