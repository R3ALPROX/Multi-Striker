...
async function getQuarantineRole(guild) {
  ...
  if (!role) {
    role = await guild.roles.create({ name: "Quarantined", permissions: [], reason: "Multi Striker quarantine system" });
  }
  return role;
}
...
async function quarantineMember(guild, memberId, reason = "Security containment") {
  ...
  if (held.has(key) || roleBackup.get(guild.id, memberId)) return { ok:true, already:true, removed:0 };

  const role = await getQuarantineRole(guild);
  if (!role.editable) return { ok:false, reason:"Quarantined role is above Multi Striker" };

  // Durable backup FIRST. This is the recovery point used after restarts as well.
  const backup = roleBackup.create(guild, member, reason);
  ...
  if (roleResult.failed.length) {
    return {
      ...
      removed:roleResult.saved.length-roleResult.failed.length,
      backupCreated:true
    };
  }
  ...
  if (!added || !refreshed || !refreshed.roles.cache.has(role.id)) {
    return { ok:false, reason:"Quarantine role could not be confirmed", backupCreated:true };
  }
  ...
  held.set(key, buildHeldRecord(guild, memberId, role.id, backup, changedChannels, reason));
  return { ok:true, removed:roleResult.saved.length, roleId:role.id, managed:roleResult.managed, backupCreated:true };
}
...
function getRecord(guild, memberId) {
  ...
  const role = guild.roles.cache.find(r => r.name === "Quarantined");
  if (!role) return null;
  const recovered = buildHeldRecord(guild, memberId, role.id, backup, [], backup.reason);
  held.set(key, recovered);
  return recovered;
}
...
async function restoreFromRecord(guild, memberId, reason, restricted) {
  ...
  if (!restricted) {
    held.delete(key);
    roleBackup.remove(guild.id, memberId);
  }
  return { ok:true, restored:restored.length, withheld, quarantined:restricted };
}

// Full release requires explicit server-owner authorization.
...
// Restricted release is deliberately separate: it may restore only non-moderation roles.
...
function isQuarantined(guildId, memberId) { return !!held.get(guildId + ":" + memberId) || !!roleBackup.get(guildId, memberId); }
