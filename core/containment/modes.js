...
function recordJoin(guild){const cfg=getGuildConfig(guild.id);const now=Date.now();const a=joins.get(guild.id)||[];a.push(now);const fresh=a.filter(t=>now-t<=cfg.security.raidWindowMs);joins.set(guild.id,fresh);return fresh.length;}
function tick(guild){const s=state(guild);if(s.until&&Date.now()>s.until){s.panic=false;s.raid=false;s.until=0;s.reason=null;states.set(guild.id,s);}return s;}
